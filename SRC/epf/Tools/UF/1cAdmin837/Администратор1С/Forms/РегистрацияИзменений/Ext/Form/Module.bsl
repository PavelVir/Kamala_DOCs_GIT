...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если Ошибка<>"" Тогда 
		ПредупреждениеСообщение(, Ошибка);
		Возврат;
	КонецЕсли;
	...
	Параметр.Вставить("ВыбранноеЗначение"	, ВыбранноеЗначение);
	...
	Если ИспользоватьРежимМодальности() Тогда
		...
		ВызватьОбработкуОтветаПриОбработкеВыбора(Ответ, Параметр);
	Иначе
		// НеСтандартный Вопрос() с обработкой результата в немодальном режиме (8.3).
		Выполнить("ОписаниеОповещенияОЗавершении = Новый ОписаниеОповещения(""ВызватьОбработкуОтветаПриОбработкеВыбора"", ЭтотОбъект, Параметр)");
...
&НаКлиенте
Процедура ВызватьОбработкуОтветаПриОбработкеВыбора(Ответ, Параметр) Экспорт
	...
	ВыбранноеЗначение = Параметр.ВыбранноеЗначение;
...
Процедура РедактироватьНомерСообщенияКонстанты()
	...
	Параметр = Новый Структура;
	Параметр.Вставить("ТекДанные", ТекДанные);
	
	Если ИспользоватьРежимМодальности() Тогда
		// Стандартный Вопрос() в модальном режиме (8.2/8.3).
		ВвестиЧисло(НомерСообщения, Подсказка);
		ВызватьОбработкуОтветаПриРедактированииНомераСообщенияКонстанты(НомерСообщения, Параметр);
	Иначе
		// НеСтандартный Вопрос() с обработкой результата в немодальном режиме (8.3).
		Выполнить("ОписаниеОповещенияОЗавершении = Новый ОписаниеОповещения(""ВызватьОбработкуОтветаПриРедактированииНомераСообщенияКонстанты"", ЭтотОбъект, Параметр)");
		Выполнить("ПоказатьВводЧисла(ОписаниеОповещенияОЗавершении, НомерСообщения, Подсказка)");
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ВызватьОбработкуОтветаПриРедактированииНомераСообщенияКонстанты(НомерСообщения, Параметр) Экспорт
	Перем ТекДанные;
...
Процедура РедактироватьНомерСообщенияСсылки()
	...
	Если ИспользоватьРежимМодальности() Тогда
		// Стандартный Вопрос() в модальном режиме (8.2/8.3).
		ВвестиЧисло(НомерСообщения, Подсказка);
		ВызватьОбработкуОтветаПриРедактированииНомераСообщенияСсылки(НомерСообщения, Параметр);
	Иначе
		// НеСтандартный Вопрос() с обработкой результата в немодальном режиме (8.3).
		Выполнить("ОписаниеОповещенияОЗавершении = Новый ОписаниеОповещения(""ВызватьОбработкуОтветаПриРедактированииНомераСообщенияСсылки"", ЭтотОбъект, Параметр)");
		Выполнить("ПоказатьВводЧисла(ОписаниеОповещенияОЗавершении, НомерСообщения, Подсказка)");
	КонецЕсли;
...
&НаКлиенте
Процедура ВызватьОбработкуОтветаПриРедактированииНомераСообщенияСсылки(НомерСообщения, Параметр) Экспорт
	Перем ТекДанные;
...
Процедура РедактироватьНомерСообщенияСписокНаборов()
	...
	Если ИспользоватьРежимМодальности() Тогда
		// Стандартный Вопрос() в модальном режиме (8.2/8.3).
		ВвестиЧисло(НомерСообщения, Подсказка);
		ВызватьОбработкуОтветаПриРедактированииНомераСообщенияСпискаНаборов(НомерСообщения, Параметр);
	Иначе
		// НеСтандартный Вопрос() с обработкой результата в немодальном режиме (8.3).
		Выполнить("ОписаниеОповещенияОЗавершении = Новый ОписаниеОповещения(""ВызватьОбработкуОтветаПриРедактированииНомераСообщенияСпискаНаборов"", ЭтотОбъект, Параметр)");
		Выполнить("ПоказатьВводЧисла(ОписаниеОповещенияОЗавершении, НомерСообщения, Подсказка)");
	КонецЕсли;
...
&НаКлиенте
Процедура ВызватьОбработкуОтветаПриРедактированииНомераСообщенияСпискаНаборов(НомерСообщения, Параметр) Экспорт
	Перем ТекДанные;
...
Процедура СообщитьОРезультатахРегистрации(Команда, Результаты)
	...
	Если Предупреждением Тогда
		ПредупреждениеСообщение(, ТекстПредупреждения,,ЗаголовокПредупреждения);
	Иначе
...
Процедура УдалитьРегистрациюКонстантыВСписке()
	...
	Параметр = Новый Структура;
	Параметр.Вставить("УзелОбменаСсылка", УзелОбменаСсылка);
	Параметр.Вставить("СписокИмен"		, СписокИмен);
	
	Если ИспользоватьРежимМодальности() Тогда
		// Стандартный Вопрос() в модальном режиме (8.2/8.3).
		Ответ = Вопрос(Текст, РежимДиалогаВопрос.ДаНет, 30, КодВозвратаДиалога.Нет, ЗаголовокВопроса, КодВозвратаДиалога.Нет);
		ВызватьОбработкуОтветаПриУдаленииРегистрацииКонстантыВСписке(Ответ, Параметр);
	Иначе
		// НеСтандартный Вопрос() с обработкой результата в немодальном режиме (8.3).
		Выполнить("ОписаниеОповещенияОЗавершении = Новый ОписаниеОповещения(""ВызватьОбработкуОтветаПриУдаленииРегистрацииКонстантыВСписке"", ЭтотОбъект, Параметр)");
		Вопрос83(Текст, 30, ОписаниеОповещенияОЗавершении);
	КонецЕсли;
...
&НаКлиенте
Процедура ВызватьОбработкуОтветаПриУдаленииРегистрацииКонстантыВСписке(Ответ, Параметр) Экспорт
	Перем УзелОбменаСсылка, СписокИмен;
	Перем ЗначениеОтвета;
	...
	УзелОбменаСсылка = Параметр.УзелОбменаСсылка;
	СписокИмен = Параметр.СписокИмен;
...
Процедура ДобавитьРегистрациюУдаленияОбъектаВСписокСсылок()
	Ссылка = КонструкторУдаленияОбъекта();
	ОбработкаВыбораДанных(Элементы.СписокСсылок, Ссылка);
КонецПроцедуры

&НаСервере
Функция КонструкторУдаленияОбъекта()
	Описание = ОбъектОбработка().ХарактеристикиПоМетаданным(СписокСсылок.ОсновнаяТаблица);
	Ссылка = Описание.Менеджер.ПолучитьСсылку();
	Возврат Ссылка;
КонецФункции
...
Процедура УдалитьРегистрациюИзСпискаСсылок()
	...
	Параметр = Новый Структура;
	Параметр.Вставить("УзелОбменаСсылка", УзелОбменаСсылка);
	Параметр.Вставить("СписокУдаления"	, СписокУдаления);
	
	Если ИспользоватьРежимМодальности() Тогда
		// Стандартный Вопрос() в модальном режиме (8.2/8.3).
		Ответ = Вопрос(Текст, РежимДиалогаВопрос.ДаНет, 30, КодВозвратаДиалога.Нет, ЗаголовокВопроса, КодВозвратаДиалога.Нет);
		ВызватьОбработкуОтветаПриОбработкеУдаленийРегистрацийИзСпискаСсылок(Ответ, Параметр);
	Иначе
		// НеСтандартный Вопрос() с обработкой результата в немодальном режиме (8.3).
		Выполнить("ОписаниеОповещенияОЗавершении = Новый ОписаниеОповещения(""ВызватьОбработкуОтветаПриОбработкеУдаленийРегистрацийИзСпискаСсылок"", ЭтотОбъект, Параметр)");
		Вопрос83(Текст, 30, ОписаниеОповещенияОЗавершении);
	КонецЕсли;
...
&НаКлиенте
Процедура ВызватьОбработкуОтветаПриОбработкеУдаленийРегистрацийИзСпискаСсылок(Ответ, Параметр) Экспорт
	Перем УзелОбменаСсылка, СписокУдаления;
	Перем ЗначениеОтвета;
	...
	УзелОбменаСсылка = Параметр.УзелОбменаСсылка;
	СписокУдаления	 = Параметр.СписокУдаления;
...
Процедура ДобавитьРегистрациюВыделенныхОбъектов(БезУчетаАвторегистрации=Истина)
	Перем ОписаниеОповещенияОЗавершении, Параметр;
	...
	Параметр = Новый Структура;
	Параметр.Вставить("БезУчетаАвторегистрации"	, БезУчетаАвторегистрации);
	Параметр.Вставить("Данные"					, Данные);
	
	Если ИспользоватьРежимМодальности() Тогда
		// Стандартный Вопрос() в модальном режиме (8.2/8.3).
		Ответ = Вопрос(Текст, РежимДиалогаВопрос.ДаНет, 30, КодВозвратаДиалога.Нет, ЗаголовокВопроса, КодВозвратаДиалога.Нет);
		ВызватьОбработкуОтветаПриОбработкеРегистрацииВыделенныхОбъектов(Ответ, Параметр);
	Иначе
		// НеСтандартный Вопрос() с обработкой результата в немодальном режиме (8.3).
		Выполнить("ОписаниеОповещенияОЗавершении = Новый ОписаниеОповещения(""ВызватьОбработкуОтветаПриОбработкеРегистрацииВыделенныхОбъектов"", ЭтотОбъект, Параметр)");
		Вопрос83(Текст, 30, ОписаниеОповещенияОЗавершении);
	КонецЕсли;
...
&НаКлиенте
Процедура ВызватьОбработкуОтветаПриОбработкеРегистрацииВыделенныхОбъектов(Ответ, Параметр) Экспорт
	Перем БезУчетаАвторегистрации, Данные;
	Перем ЗначениеОтвета;
	...
	БезУчетаАвторегистрации = Параметр.БезУчетаАвторегистрации;
	Данные = Параметр.Данные;
...
Процедура УдалитьРегистрациюВыделенныхОбъектов(БезУчетаАвторегистрации=Истина)
	...
	Если ИспользоватьРежимМодальности() Тогда
		// Стандартный Вопрос() в модальном режиме (8.2/8.3).
		Ответ = Вопрос(Текст, РежимДиалогаВопрос.ДаНет, 30, КодВозвратаДиалога.Нет, ЗаголовокВопроса, КодВозвратаДиалога.Нет);
		ВызватьОбработкуОтветаПриУдаленииРегистрацииВыделенныхОбъектов(Ответ, Параметр);
	Иначе
		// НеСтандартный Вопрос() с обработкой результата в немодальном режиме (8.3).
		Выполнить("ОписаниеОповещенияОЗавершении = Новый ОписаниеОповещения(""ВызватьОбработкуОтветаПриУдаленииРегистрацииВыделенныхОбъектов"", ЭтотОбъект, Параметр)");
		Вопрос83(Текст, 30, ОписаниеОповещенияОЗавершении);
	КонецЕсли;
...
&НаКлиенте
Процедура ВызватьОбработкуОтветаПриУдаленииРегистрацииВыделенныхОбъектов(Ответ, Параметр) Экспорт
	Перем БезУчетаАвторегистрации, УзелОбменаСсылка, Данные;
	Перем ЗначениеОтвета;
...
Процедура ОбработкаВыбораДанных(ТаблицаФормы, ВыбранноеЗначение)
	...
	Иначе
		...
		ЗаголовокВопроса = "Подтверждение";
		
		Параметр = Новый Структура;
		Параметр.Вставить("УзелОбменаСсылка", УзелОбменаСсылка);
		Параметр.Вставить("Данные"			, Данные);
		Параметр.Вставить("ТаблицаФормы"	, ТаблицаФормы);
		Параметр.Вставить("ИмяТаблицы"		, ИмяТаблицы);
		Параметр.Вставить("Действие"		, Действие);
		Параметр.Вставить("Ссылка"			, Ссылка);
		
		Если ИспользоватьРежимМодальности() Тогда
			// Стандартный Вопрос() в модальном режиме (8.2/8.3).
			Ответ = Вопрос(Текст, РежимДиалогаВопрос.ДаНет, 30, КодВозвратаДиалога.Нет, ЗаголовокВопроса, КодВозвратаДиалога.Нет);
			ВызватьОбработкуОтветаПриОбработкеВыбораДанных(Ответ, Параметр);
		Иначе
			// НеСтандартный Вопрос() с обработкой результата в немодальном режиме (8.3).
			Выполнить("ОписаниеОповещенияОЗавершении = Новый ОписаниеОповещения(""ВызватьОбработкуОтветаПриОбработкеВыбораДанных"", ЭтотОбъект, Параметр)");
			Вопрос83(Текст, 30, ОписаниеОповещенияОЗавершении);
		КонецЕсли;
...
&НаКлиенте
Процедура ВызватьОбработкуОтветаПриОбработкеВыбораДанных(Ответ, Параметр) Экспорт
	Перем УзелОбменаСсылка, Данные, ТаблицаФормы, ИмяТаблицы, Действие, Ссылка;
	Перем Результат;
	Перем ЗначениеОтвета;
	...
	УзелОбменаСсылка = Параметр.УзелОбменаСсылка;
	Данные			 = Параметр.Данные;
	ТаблицаФормы	 = Параметр.ТаблицаФормы;
...
Процедура ОперацияСРезультатамиЗапроса(КомандаОперации)
	...
	Если ИспользоватьРежимМодальности() Тогда
		...
		ВызватьОбработкуОтветаПриОбработкеРезультатаЗапроса(Ответ, Неопределено);
	Иначе
		// НеСтандартный Вопрос() с обработкой результата в немодальном режиме (8.3).
		Выполнить("ОписаниеОповещенияОЗавершении = Новый ОписаниеОповещения(""ВызватьОбработкуОтветаПриОбработкеРезультатаЗапроса"", ЭтотОбъект, Неопределено)");
...
&НаКлиенте
Процедура ВызватьОбработкуОтветаПриОбработкеРезультатаЗапроса(Ответ, Параметр) Экспорт
	Перем ЗначениеОтвета;
...
Процедура ПрочитатьДеревоМетаданных()
	Данные = ОбъектОбработка().СформироватьСтруктуруМетаданных(УзелОбменаСсылка);
...
Процедура ЗаполнитьКоличествоРегистрацийВДереве()
	...
	Данные = ОбъектОбработка().ПолучитьКоличествоИзменений(СтруктураИменМетаданных, УзелОбменаСсылка);
...
Процедура НастроитьСписокСсылок(ИмяТаблицы, Наименование)
	...
	Мета = ОбъектОбработка().МетаданныеПоПолномуИмени(ИмяТаблицы);
...
Процедура НастроитьНаборЗаписей(ИмяТаблицы, Измерения, Наименование)
	...
	ОбъектОбработка().ДобавитьКолонкиВТаблицуФормы(
...
Функция МассивИменКлючейНабораЗаписей(ИмяТаблицы, ПрефиксИмен="")
	...
	Измерения = ОбъектОбработка().ИзмеренияНабораЗаписей(ИмяТаблицы);
	Если Измерения<>Неопределено Тогда
		Для Каждого Строка Из Измерения Цикл
...
Функция МенеджерПоМетаданным(ИмяТаблицы) 
	Описание = ОбъектОбработка().ХарактеристикиПоМетаданным(ИмяТаблицы);
...
Функция УдалитьРегистрациюНаСервере(БезУчетаАвторегистрации, Узел, Удаляемые, ИмяТаблицы=Неопределено)
	Возврат ОбъектОбработка().ИзменитьРегистрациюНаСервере(Ложь, БезУчетаАвторегистрации, Узел, Удаляемые, ИмяТаблицы);
КонецФункции

&НаСервере
Функция ДобавитьРегистрациюНаСервере(БезУчетаАвторегистрации, Узел, Добавляемые, ИмяТаблицы=Неопределено)
	Возврат ОбъектОбработка().ИзменитьРегистрациюНаСервере(Истина, БезУчетаАвторегистрации, Узел, Добавляемые, ИмяТаблицы);
КонецФункции
...
Функция ПрочитатьНомераСообщений()
	...
	Данные = ОбъектОбработка().ПолучитьПараметрыУзлаОбмена(УзелОбменаСсылка, РеквизитыЗапроса);
	Если Данные=Неопределено Тогда
...
Процедура ОбработатьЗапретИзмененияУзла()
	...
	Если ОперацииРазрешены Тогда
		...
		Заголовок = НСтр("ru='1С:Администратор: Регистрация изменений объектов в планах обмена'");
	Иначе
		Элементы.УзелОбменаСсылка.Видимость = Ложь;
		Заголовок = СтрЗаменить(НСтр("ru='1С:Администратор: Регистрация изменений объектов в плане обмена ""%1""'"), "%1", Строка(УзелОбменаСсылка));
	КонецЕсли;
...
Функция СтруктураКлючаНабораЗаписей(Знач ТекущиеДанные)
	Описание  = ОбъектОбработка().ХарактеристикиПоМетаданным(СписокНаборовЗаписейИмяТаблицы);
