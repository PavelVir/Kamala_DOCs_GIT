	...
	Для каждого Зн Из НеУникальныеНомера Цикл
		...
		Если ЭтаФорма.НеПрерыватьВыполнениеПриОшибке Тогда
			Попытка      
				Объект.Записать();
			Исключение
				Сообщить(НСтр("ru = 'Ошибка записи '") + Строка(Объект.Ссылка) + ": " + ИнформацияОбОшибке());
...
Функция ВыполнитьОбработку() Экспорт
	...
	Если (НачальныйНомер = 0) И (Не НеИзменятьЧисловуюНумерацию) Тогда
		Сообщить("Измените начальный номер!");
		Возврат 0;
	КонецЕсли;
...
мНастройка = Новый Структура("НачальныйНомер,НеИзменятьЧисловуюНумерацию,СтрокаПрефикса,ЗаменяемаяПодстрока,СпособОбработкиПрефиксов, РежимРаботы, ИспользоватьРежимЗагрузкиОбменаДанными, НеПрерыватьВыполнениеПриОшибке");
